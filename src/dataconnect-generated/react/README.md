...
## ListTasksForFarm
You can execute the `ListTasksForFarm` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useListTasksForFarm(dc: DataConnect, vars: ListTasksForFarmVariables, options?: useDataConnectQueryOptions<ListTasksForFarmData>): UseDataConnectQueryResult<ListTasksForFarmData, ListTasksForFarmVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListTasksForFarm(vars: ListTasksForFarmVariables, options?: useDataConnectQueryOptions<ListTasksForFarmData>): UseDataConnectQueryResult<ListTasksForFarmData, ListTasksForFarmVariables>;
```
...
```
### Return Type
Recall that calling the `ListTasksForFarm` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListTasksForFarm` Query is of type `ListTasksForFarmData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListTasksForFarm`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, ListTasksForFarmVariables } from '@dataconnect/generated';
import { useListTasksForFarm } from '@dataconnect/generated/react'

export default function ListTasksForFarmComponent() {
  // The `useListTasksForFarm` Query hook requires an argument of type `ListTasksForFarmVariables`:
  ...
  const query = useListTasksForFarm(listTasksForFarmVars);
  // Variables can be defined inline as well.
  const query = useListTasksForFarm({ farmId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListTasksForFarm(dataConnect, listTasksForFarmVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useListTasksForFarm(listTasksForFarmVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useListTasksForFarm(dataConnect, listTasksForFarmVars, options);
...
You can execute the `GetCropBatchesForFarm` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetCropBatchesForFarm(dc: DataConnect, vars: GetCropBatchesForFarmVariables, options?: useDataConnectQueryOptions<GetCropBatchesForFarmData>): UseDataConnectQueryResult<GetCropBatchesForFarmData, GetCropBatchesForFarmVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetCropBatchesForFarm(vars: GetCropBatchesForFarmVariables, options?: useDataConnectQueryOptions<GetCropBatchesForFarmData>): UseDataConnectQueryResult<GetCropBatchesForFarmData, GetCropBatchesForFarmVariables>;
```
...
```
### Return Type
Recall that calling the `GetCropBatchesForFarm` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetCropBatchesForFarm` Query is of type `GetCropBatchesForFarmData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetCropBatchesForFarm`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetCropBatchesForFarmVariables } from '@dataconnect/generated';
import { useGetCropBatchesForFarm } from '@dataconnect/generated/react'

export default function GetCropBatchesForFarmComponent() {
  // The `useGetCropBatchesForFarm` Query hook requires an argument of type `GetCropBatchesForFarmVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetCropBatchesForFarm(getCropBatchesForFarmVars);
  // Variables can be defined inline as well.
  const query = useGetCropBatchesForFarm({ farmId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetCropBatchesForFarm(dataConnect, getCropBatchesForFarmVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetCropBatchesForFarm(getCropBatchesForFarmVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetCropBatchesForFarm(dataConnect, getCropBatchesForFarmVars, options);
  ...
  // If the Query is successful, you can access the data returned using the `UseQueryResult.data` field.
  if (query.isSuccess) {
    console.log(query.data.cropBatches);
  }
  return <div>Query execution {query.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
...
export default function CreateNewTaskComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ title: ..., status: ..., dueDate: ..., farmId: ..., cropBatchId: ..., assignedToId: ..., });
