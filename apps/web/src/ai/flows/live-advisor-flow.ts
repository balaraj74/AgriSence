...
try {
  ...
  let inlineData;
  const match = input.videoFrameUri.match(/^data:(.+);base64,(.+)$/);
  if (match) {
    inlineData = { mimeType: match[1], data: match[2] };
  } else {
    inlineData = { mimeType: 'image/jpeg', data: input.videoFrameUri.split(',')[1] || input.videoFrameUri };
  }
  ...
  const output = await callGemini(promptPayload, {
        ...
        responseSchema: LiveFarmAdvisorOutputSchema,
      });
